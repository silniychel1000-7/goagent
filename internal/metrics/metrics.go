...
type Metrics struct {
	...
	// extra exposition blobs injected from external collectors
	mu    sync.RWMutex
	extra []func() string
}
...
func New() *Metrics {
	return &Metrics{
		...
		jitter:     newGauge("network_jitter_ms", "RTT jitter (mean absolute deviation) in milliseconds."),
	}
}

// RegisterExtra registers an exposition provider (e.g. interface collector, TCP probe).
// Thread-safe; call before serving /metrics or from init goroutine.
func (m *Metrics) RegisterExtra(fn func() string) {
	m.mu.Lock()
...
func (m *Metrics) Update(target string, up bool, latencyMs, packetLoss, jitterMs float64) {
	upVal := 0.0
	if up {
		upVal = 1.0
	}
