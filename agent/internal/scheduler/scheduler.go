...
// check pings a single target and updates metrics + logs.
func (s *Scheduler) check(target string) {
	result := s.pinger.Ping(target)

	s.metrics.Update(target, result.Up, result.LatencyMs, result.PacketLoss)
	...
	default:
		s.log.Debug("ping ok", "target", target, "latency_ms", result.LatencyMs, "packet_loss", result.PacketLoss)
	}
}
